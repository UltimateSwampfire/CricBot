...
pts_lst = [] # list of (x,y,z) coords
x_data = np.array([])
y_data = np.array([])
z_data = np.array([])
x_parametric = np.array([])
y_parametric = np.array([])
z_parametric = np.array([])
# Create pipeline
           ...
           # spatialData = synced['spatialData'].getSpatialLocations()
            ...
            if curr_point != (0,0,0):
                ...
                if  curr_point[2] < max_distance:
                    ...
                    if len(pts_lst)>2:
                        x_data = np.append(x_data, curr_point[0])
                        y_data = np.append(y_data, curr_point[1]) #Global Height (vertical)
                        z_data = np.append(z_data, curr_point[2]) #Global Depth (along pitch)
                        #Calculating parameters
                        if len(x_data)>5:
                            constants_1 = curve_fit(plane,x_data, z_data,maxfev=5000)
                            ...
                            constants_2 = curve_fit(profile,z_data,y_data,maxfev=5000)
...
ax[0][0].set_ylim(min(z_data),max(z_data))
...
ax[0][1].set_ylim(min(y_data),max(y_data))
